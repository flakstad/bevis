...
3. Keep the existing advisory transaction lock and five-per-hour identity
   ...
   `authn.policy/issuance-decision`.
4. Move only token/code/session mechanics to authn-core. Keep normalization,
...
5. Retain the current CSRF model initially; authn-core does not absorb it.
   A later change may hash the CSRF token at rest using `authn.secret`, but that
