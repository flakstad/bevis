...
`authn.secret/matches?` uses `MessageDigest/isEqual` for in-process digest
...
`authn.policy/issuance-decision` consumes counts supplied from durable storage.
...
`authn.ring/session-cookie` defaults to `Secure`, `HttpOnly`, `SameSite=Strict`,
